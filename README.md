### Tarea 1 Inteligencia Artificial: Escape del Laberinto Mutante

Grupo:
Pablo Villagrán Hermanns
Matrícula:
2023439231
...
En caso de que usted profesora desee evaluar cada algoritmo por su cuenta lo que debe hacer es correr el archivo Main.py. Para esto debe clonar el repositorio, estar situado en la carpeta raíz, y correr el siguieinte comando, remplanzado cada eje por los valores que desee: 
 ```bash 
  python Main.py <tamaño_lab> <cant_salidas> <algoritmo> <densidad> 
 ```
esto iniciara el proceso de crear el laberinto y lo solucionara con el algoritmo que desee(el parámetro algoritmo debe ser uno de los antes mencionado y con la nomenclatura dicha en el párrafo anterior).
...
## A Star
Los resultados de este algoritmo fueron los mejores, obtuvo una gran cantidad de tiempos muy menores y constantes en todos los casos, y el porcentaje de escape fue muy mayor al igual que el anterior.

![Resultados Astar](images/Astar.png)

Los resultados tan mayores se deben a que la heurística usada, la cual permite acercarse a la salida cada vez mas, y como tiene la opción de esperar a que el laberinto cambie, se puede adaptar facilmente al entorno movil.

![Aciertos Astar](images/succes_Astar.png)
![Aciertos Astar](images/success_Astar_5.png)
![Aciertos Astar](images/success_Astar_10.png)
![Aciertos Astar](images/succes_Astar_50.png)
![Aciertos Astar](images/success_100_Astar.png)

Podemos notar el porcentaje de aciertos es siempre muy superior a la de fallos, dandole robustes al algoritmo.

## Algoritmo Genético

Este algoritmo tuvo muy buenos resultados en tiempo, pero no tan buenos en capacidad de escapar. Esto estimamos que es por la cantidad de caminos disponibles y por la cantidad de generaciones que le permitimos al algoritmo, que dado su aleatoriedad, no logra llegar a la ruta correcta antes de que se acaben los intentos, y como el laberinto es capaz de cambier, pierde parte de su progreso obtenido en cada iteración.

![Resultados GEN](images/gen.png)

Notamos que los tiempos son constantes, y a lo largo de los tamaños van creciendo dado que el largo de los caminos es mayor, por lo que se demora mas en evaluarlos todos y en crear nuevos caminos.

![Aciertos GEN](images/gen_succes.png)

En este caso no se ve pero el porcentaje de aciertos cuando el laberinto es de tamaño 5 es del 100%, demostrando su capacidad de resolver estos problemas mas pequeños en poco tiempo y con alta efectividad.

![Aciertos GEN](images/success_gen_5.png)
![Aciertos GEN](images/succes_gen_10.png)
![Aciertos GEN](images/success_gen_50.png)
![Aciertos GEN](images/succes_gen_100.png)


## Reflexiones
Una vez evaluados todos los algoritmos nos dimos cuenta que el mejor de todos es el al algoritmos informado A Star, dado que, al poseer una buena heurística, este tiene lo necesarios para llegar la mayoría de las veces a la salida, y dado su velocidad y capacidad de esperar, junto con el bajo consumo de recursos al no guardar todo el camino, permite resolver problemas de este tipo de una forma facil y rápida sin complicaciones. El segundo lugar se disputa entre los otros dos, dado que, si la capacidad de computo fuese mayor, se podrían incrementar la cantidad de generaciones y de caminos evaluados en el algoritmo genético, permitiendo un mejor uso de este y un crecimiento en los resutados positivos, pero a su vez, el algoritmo bfs puede tener muy buenos resultados y con muchos más aciertos en laberintos pequeños, por lo que dependería de la complejidad del laberinto y de su tamaño el algoritmo que convenga implementar. 
